class Spree::Comment < ActiveRecord::Base
  ...
  def send_email
  	if self.commentable.class == Spree::Shipment
  		...
  		others = self.user.admin? ? [self.commentable.stock_location.supplier.users.first,self.commentable.order.user] : (self.user == self.commentable.order.user) ? [self.commentable.stock_location.supplier.users.first] : [self.commentable.order.user]
